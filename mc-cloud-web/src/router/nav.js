...
function parseRouters(menus, routers) {
  menus.forEach(item => {
    if (item.url || item.menu) {
      routers.push({
        path: item.url,
        name: item.name,
        component: () => import('@/view' + item.url + '.vue'),
        meta: {
          auth: item.auth || true,
          keepAlive: true,
          menuActive: item.menu ? item.url: item.parent,
          ...item
        }
      });
      //处理功能点
      if (item.actions) {
        parseRouters(item.actions, routers);
      }
    } else if (item.subset) {
      parseRouters(item.subset, routers);
    }
  })
}
...
export const getMenuList = function () {
  let menus = store.getters.userInfo.authorities; //权限数据
  let menuList = [];
  navArr.forEach(item => {
    if ((item.url || item.menu) && menus.indexOf(item.id) > -1) menuList.push(item);
    if (item.subset) {
      let subsets = item.subset.filter(m => menus.indexOf(m.id) > -1);
...
//生成菜单的vue路由
export const getMenuRouter = function () {
  let menuRouter = [];
  parseRouters(getMenuList(), menuRouter);
  menuRouter.push({ path: '*', redirect: '/404', hidden: true });//最后面的404页面
  return menuRouter;
};
