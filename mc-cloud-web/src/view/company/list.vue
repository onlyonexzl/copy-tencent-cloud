    ...
    <div class="item-box padd20 table-search-box clearfix">
      <el-form ref="searchForm" :model="searchForm" label-width="100px">
        <el-form-item label="名称">
          <el-input v-model="searchForm.name" placeholder="服务商名称" style="width: 250px;"></el-input>
        </el-form-item>
        ...
        <el-button type="primary" icon="el-icon-search" class="btn-search" @click="submitSearchForm('searchForm')">搜索
        </el-button>
      </el-form>
    </div>
    <div class="item-box">
      ...
      <el-table
        ...
        <el-table-column prop="orgCode" label="机构代码" width="200">
          <template slot-scope="scope">
            <span>{{ scope.row.orgCode }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="linkName" label="联系人姓名" width="150">
          <template slot-scope="scope">
            <span>{{ scope.row.linkName }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="linkQq" label="QQ" width="150">
          <template slot-scope="scope">
            <span>{{ scope.row.linkQq }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="linkPhone" label="手机号" width="150">
          <template slot-scope="scope">
            <span>{{ scope.row.linkPhone }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="linkEmail" label="邮箱" width="250">
          <template slot-scope="scope">
...
<script>
  ...
  export default {
    ...
    methods: {
      queryPage() {
        this.loading = true;
        providerService.queryPage(this.currentPage, this.pageSize, this.searchForm).then(res => {
          if (res.data.state === 1) {
      ...
      handleEdit(index, row) {
        this.$router.push({path: '/provider/edit', query: {id: row.id}})
      },
      handleDelete(index, row) {
        this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
          ...
          providerService.deleteById(row.id).then(res => {
            this.$message({
              type: 'success',
              message: '删除成功!'
            });
