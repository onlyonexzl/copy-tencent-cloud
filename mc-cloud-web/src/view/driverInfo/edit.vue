...
<script>
    ...
    export default {
            ...
            onSubmit(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        driverInfoService.update(this.form).then(res => {
                            if (res.data.state === 1) {
        ...
        created() {
            ...
            driverInfoService.getById(id).then(res => {
                if (res.data.state === 1) {
                    const data = res.data.data;
                    const that = this;
                    this.form.profileId = res.data.data.profileId;
                    this.initDriverAttributs(function () {
                        that.form = data;
                    });
                } else {
                    throw new Error(res.data.msg);
