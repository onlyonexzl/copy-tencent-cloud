...
<script>
    ...
    export default {
        ...
        methods: {
            queryPage() {
                this.loading = true;
                pointAttributeService.queryPage(this.currentPage, this.pageSize, this.searchForm).then(res => {
                    if (res.data.state === 1) {
            ...
            handleAdd(index, row) {
                this.$router.push({path: '/pointAttribute/add'})
            },
            handleEdit(index, row) {
                this.$router.push({path: '/pointAttribute/edit', query: {id: row.id}})
            },
            handleDelete(index, row) {
                this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
                    ...
                    pointAttributeService.deleteById(row.id).then(res => {
                        this.$message({
                            type: 'success',
                            message: '删除成功!'
                        });
