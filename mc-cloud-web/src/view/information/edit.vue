...
<script>
    ...
    export default {
            ...
            onSubmit(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        informationService.update(this.form).then(res => {
                            if (res.data.state === 1) {
        ...
        created() {
          ...
          informationService.getById(id).then(res => {
            if (res.data.state === 1) {
              let d = res.data.data;
              return new Promise((resolve, reject) => {
                infoTypeService.queryList().then(res => {
                  if (res.data.state === 1) {
                      resolve([d, res.data.data]);
                  } else {
                    reject(res.data.msg);
                  }
                }).catch(error => {
                  this.$message.error("类型加载失败");
                })
              })
            } else {
              throw new Error(res.data.msg);
            ...
            this.form = d;
            this.types = types;
            this.editor.txt.html(d.content);
          }).catch(error => {
            this.$message.error(error.message);
          }).finally(() => {
